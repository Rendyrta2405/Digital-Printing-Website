  ...
  <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4">
        ...
        <div class="col">
            <div class="card h-100 shadow-sm border-0">
                ...
                <div class="position-relative overflow-hidden" 
                   style="padding-top: 75%; background-color: #f8f9fa;">
                    <img src="{{ asset('storage/' . $p->image) }}" 
                         data-name="{{ $p->name ?? 'Tidak ada deskripsi' }}"
                ...
                <div class="card-body d-flex flex-column justify-content-between p-3">
                    <p class="card-text text-muted small text-truncate mb-3" title="{{ $p->name }}">
                        {{ $p->name ?? 'Tidak ada Nama' }}
                    </p>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <!-- Form Hapus -->
                        <form action="{{ route('admin.partners.destroy', $p) }}" 
                              method="POST" 
    ...
    @empty
        <div class="p-6 w-full text-center text-slate-400">
           ...
           <br>
           <span>Anda belum memiliki partner!!</span>
           <br>
           <span>Ayo bekerja sama dengan bisnis / perusahaan lain untuk menambahkan Partner Anda</span>
        </div>
    @endforelse
</div>
...
<div class="modal fade" id="AddModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
     <form action="{{ route('admin.partners.store') }}" method="post" class="form" enctype="multipart/form-data">
        @csrf
       <div class="modal-content">
         <div class="modal-header">
           <h1 class="modal-title fs-5" id="exampleModalLabel">🛠️ Tambah Partner</h1>
         ...
         <div class="modal-body">
            ...
            <div class="mb-3">
               <label for="" class="form-label">Tampilkan Partner di website?</label>
...
<div class="modal fade" id="EditModal{{ $p->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
     <form action="{{ route('admin.partners.update', $p) }}" method="post" class="form" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
       <div class="modal-content">
         <div class="modal-header">
           <h1 class="modal-title fs-5" id="exampleModalLabel">🛠️ Update Partner</h1>
           <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
         </div>
         <div class="modal-body">
            <div class="">
            ...
            <div class="mb-3">
               ...
               <select name="show_in_web" id="" class="form-select">
                  <option value="1"
                     ...
                     $p->show_in_web ?? ''))>Tampilkan</option>
                  <option value="0"
                     @selected(old('show_in_web', 
                     !$p->show_in_web ?? ''))>Tidak</option>
               </select>
            </div>
         </div>
         <div class="modal-footer">
